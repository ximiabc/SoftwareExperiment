...
<body>
	...
	<div class="rightinfo">
			...
			<tbody>
				<s:iterator value="pageBean.recordList" var="n" status="indexs">
					<tr>
						...
						<td><a href="Managecenterview?id=${n.id }">${n.titles }</a>
						</td>
						<td>${n.count }</td>
						<td>${n.date }</td>
						<td><c:if test="${n.view == true }">
								<a href="Managecenterunshow?id=${n.id }">关闭</a>
							</c:if> 
							<c:if test="${n.view != true }">
								<a href="Managecentershow?id=${n.id }">开启</a>
							</c:if>
						</td>
					</tr>
				</s:iterator>
		...
		<div class="pageing">
			<ul class="page">
				<li><a href="Managecenterupdate?pageNum=${pageNums -1}"><</a></li>
				<s:iterator begin="pageBean.beginPageIndex"
					...
					<li
						...
						<c:if test="${pageBean.currentPage == pageNums }">
							...
							<a href="Managecenterupdate?pageNum=${pageNums }">${pageNums }</a>
						</c:if>
					</li>
				</s:iterator>

				<li><a href="Managecenterupdate?pageNum=${pageNums }">></a></li>
			</ul>
		</div>
	</div>
